...
def run_grading_pipeline(
    ...
    _print("Uploading Master Marking Scheme to Gemini...")
    ...
    _print("Pre-scanning Marking Scheme for Total Marks and Topics...")
...
"""
    ...
    _print(f"Pre-Scan Complete! Paper Total: {paper_total_marks} marks.")
    _print(f"Standardized Topics for this paper: {approved_topics}\n")


    # --- 3. PASS 2: THE GRADING LOOP ---
    _print("Starting overall system scan...\n")
    ...
    for class_folder_name in os.listdir(base_directory):
        ...
        _print(f"=====================================")
        _print(f"Detected Class: {class_id}")
        _print(f"=====================================")

        pdf_files = glob.glob(f"{class_folder_path}/*.pdf")
        if not pdf_files:
            _print(f"No student worksheets found in {class_id}. Skipping...\n")
            continue
        ...
        _print(f"  -> Calculating analytics for {class_id}...")
        ...
        _print(f"Finished processing {class_id}.\n")
    ...
    _print(f"Pipeline complete! Master {output_path} is ready for the Figma frontend.")
...
if __name__ == "__main__":
    run_grading_pipeline(
        ...
        output_path="output/database_feed.json"
    )
